...
/**
 * An extension to {@link WorkerService}, the `TilerService` implements an asynchronous
 * (message based) service to tile untiled payloads in web workers. The `TilerService` itself lives
 * in the web worker, and communicates with messages by means of a {@link ConcurrentWorkerSet}
 * with the application.
 *
 * The `TilerService` registers tile indices (parent tile to be subdivided) by handling a
 * {@link RegisterIndexRequest}, and can later retrieve tiled payloads from through the
 * {@link TileRequest}. The data is sent back in form of a {@link WorkerServiceResponse}.
 */
export class TilerService extends WorkerService {
    /**
     * Start a {@link TilerService}.
     *
